...
def preprocess_features(features):
    ...
    features = (features - tf.constant([[0.0, 0.0, 162.5]])) / tf.constant([[20.0, 60.0, 127.5]])
...
def get_generator(activation, kernel_init, num_features, latent_dim):
    generator = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Conv2D(
                filters=8, kernel_size=(1, 3), padding='same', activation=activation, kernel_initializer=kernel_init
            ),
            tf.keras.layers.Conv2D(
                filters=8, kernel_size=(3, 2), padding='valid', activation=activation, kernel_initializer=kernel_init
            ),  # 6x15
            tf.keras.layers.Conv2D(
                filters=1,
                kernel_size=1,
...
def get_discriminator(
    ...
    if features_to_tail:
        features_tiled = tf.tile(tf.reshape(features_input, (-1, 1, 1, num_features)), (1, 6, 15, 1))
    ...
    discriminator_tail = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Conv2D(
                filters=16, kernel_size=(1, 4), padding='valid', activation=activation, kernel_initializer=kernel_init
            ),  # 6x12
            tf.keras.layers.Dropout(dropout_rate),
